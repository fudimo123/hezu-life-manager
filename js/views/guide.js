...
(function () {
  ...
  const ENTRIES = [
    ...
    { icon: '🏛️', name: '公约广场', path: '公约页顶部横幅', desc: '采用社区热门公约（含采用数）· 分享你家的好公约' },
    { icon: '🤝', name: '找室友评估', path: '成员管理 → 找室友评估', desc: '输入候选人生活习惯 → 合拍报告 + AI 意见 + 建议公约 + 邀请入住' },
    { icon: '🏠', name: '共享之家（云同步）', path: '成员管理 → 共享之家', desc: '创建/加入云端家庭（邀请码），账本·值日·库存·公约实时共享，家动态点赞评论同步' },
  ];
